...
class PbCompletionsProvider {
    ...
    init(path, ignore) {
        var includePath = child_process.spawnSync('pebble',
            ['sdk', 'include-path', 'aplite']);
        if ('error' in includePath) {
            includePath = child_process.spawnSync(
                '~/workspace/pebble/pebble-sdk/bin/pebble',
                ['sdk', 'include-path', 'aplite']);
        }
        includePath = includePath.output[1].toString('utf-8').replace('\n', '');
