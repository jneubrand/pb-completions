...
module.exports = {
    ...
    reloadSuggestions(data) {
        ...
        for (var define_match in define_matches) {
            ...
            if (temp !== null) {
                suggestions.push({
                    'snippet': temp[1] + module.exports.snippetize(temp[2]),
                    'displayText': temp[1],
                    'leftLabel': '#define',
                    'type': (temp[2] === undefined ? 'constant' : 'function')
                });
            }
        }
        ...
        for (var functions_match in function_matches) {
            ...
            if (temp !== null) {
                suggestions.push({
                    'snippet': temp[2] + module.exports.snippetize(temp[3]),
                    'displayText': temp[2],
                    'leftLabel': temp[1],
                    'type': 'function'
                });
        ...
        for (let enums_match in enum_matches) {
            ...
            let regex1 = /^\s*[^ \/\n(typedef)}].*/gm; // extract useful lines
