...
class PbCompletionsProvider {
    ...
    init(path, ignore) {
        if (path.indexOf('{{{INCLUDE}}}') != -1) {
                ...
                function(error, data) {
                    if (error) {
                        atom.notifications.addError('pb-completions: ' +
                            ...
                            'If you\'d rather skip this, simply edit the includes via' +
                            'Atom\'s preferences to avoid using the {{{INCLUDE}}} replacement.');
            ...
            this.headerLocations = path
                    .replace(/\{\{\{HOME\}\}\}/g, os.homedir())
                    .replace(/\{\{\{LIB\}\}\}/g, __dirname)
                    .split('{{{AND}}}');
            this.ignoredItems = ignore;
        }
    }
    ...
    reloadHeaders(giveUp) {
        if (!this.headerLocations) {
            console.warn('No header locations set.');
            if (giveUp !== true) {
                console.warn('Retrying in 1s.');
                var self = this;
                setTimeout(function() {self.reloadHeaders(true)}, 1000);
            } else {
                console.warn('Won\'t retry.');
            }
            return
        }
